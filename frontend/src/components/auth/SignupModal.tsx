...
const COUNTRY_OPTIONS: CountryOpt[] = [{ code: 'IN', label: 'India', dial: '+91', flag: '🇮🇳' }];
...
const TENANT_BENEFITS: Benefit[] = [
  { text: 'Browse thousands of verified properties', icon: <Home className="h-4 w-4" /> },
  { text: 'Contact owners and brokers directly at no charge', icon: <PhoneCall className="h-4 w-4" /> },
  { text: 'Save searches and get instant alerts for new listings', icon: <Zap className="h-4 w-4" /> },
  { text: 'Shortlist favorites and compare easily', icon: <Star className="h-4 w-4" /> },
  { text: 'Zero brokerage charged by EasyLease', icon: <ShieldCheck className="h-4 w-4" /> },
  { text: 'Schedule visits, get directions and chat on WhatsApp', icon: <Users className="h-4 w-4" /> },
];

const LISTER_BENEFITS: Benefit[] = [
  { text: 'List unlimited properties for free', icon: <Building2 className="h-4 w-4" /> },
  { text: 'Get discovered by thousands of tenants  -  no platform fee', icon: <Megaphone className="h-4 w-4" /> },
  { text: 'Leads in real time via call, email and WhatsApp', icon: <PhoneCall className="h-4 w-4" /> },
  { text: 'Add rich details: photos, amenities, availability and more', icon: <Star className="h-4 w-4" /> },
...
function SignupForm({ onSwitch }: SignupProps): JSX.Element {
  ...
  const emailBlocked = useMemo(() => /\bemail\b.\b(banned|blocked)\b/i.test(err), [err]);
  const phoneBlocked = useMemo(() => /\b(phone|mobile)\b.\b(banned|blocked)\b/i.test(err), [err]);
  ...
  const fullPhone = useMemo(() => `${country.dial}${phoneDigits}`, [country, phoneDigits]);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        const res = await fetch(`${API_URL}/auth/available?email=${encodeURIComponent(e)}`, { signal: controller.signal });
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        const res = await fetch(`${API_URL}/auth/available?phone=${encodeURIComponent(fullPhone)}`, { signal: controller.signal });
        const json = await res.json().catch(() => null);
  ...
  const googleLogin = useGoogleLogin({
    ...
    onSuccess: async (tokenResponse) => {
      try {
        ...
        setConfirm(autoPwd);
        setEmailLocked(true);
        setInfo('Google connected. Please select your role and add your mobile number to complete signup.');
      } catch (e: any) {
  ...
  const handleSubmit: React.FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    if (!payload.name || !payload.email || !phoneDigits || !payload.password) return setErr('Please fill all required fields');
    if (!validPhone) return setErr('Enter a valid 10-digit mobile number.');
    if (!googleMode && !emailValid) return setErr('Please enter a valid email address.');
    if (!googleMode && password !== confirm) return setErr('Passwords do not match');
    ...
    try {
      ...
      if (resp?.pending_verification) {
        const verifyData = { email: resp.email || trimmedEmail, resendUrl: resp.resend_url as string };
  ...
  const submitDisabled =
    loading || phoneChecking || emailChecking || phoneTaken || emailTaken || !validPhone || (!googleMode && !emailValid);
    ...
    <div className="w-full">
      <div className="mx-auto w-full max-w-5xl px-4 sm:px-6 lg:px-8">
        <div className="mb-6 flex flex-wrap items-center gap-2 justify-center">
          ...
          {(['tenant', 'owner', 'broker', 'builder'] as Role[]).map((r) => (<button key={r} type="button" onClick={() => setBenefitRole(r)} className={`px-3 py-1.5 rounded-full text-sm border ${benefitRole === r ? 'bg-[#2AB09C] text-white border-[#2AB09C]' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`} >
          ...
          >
            <div>
              ...
              <p className="text-gray-600 mt-1 text-center">
                {isLister ? 'Reach more tenants with every listing.' : 'Find great homes with zero brokerage.'}
              </p>
            </div>
            ...
            {err && <div className="bg-red-50 border-l-4 border-red-500 p-3 text-red-700 rounded">{err}</div>}
            {info && <div className="bg-emerald-50 border-l-4 border-emerald-500 p-3 text-emerald-700 rounded">{info}</div>}
            ...
            <div className="grid gap-5 sm:grid-cols-2">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-3 py-2.5 border ${emailTaken || (!emailValid && email) ? 'border-red-500' : 'border-gray-300'
                    } rounded-md focus:ring-2 focus:ring-[#2AB09C] outline-none ${googleMode && emailLocked ? 'bg-gray-100 text-gray-600' : ''
                    }`}
                ...
                <div className="mt-1 text-xs">
                  ...
                  {emailTaken && <div className="flex items-center gap-2 text-red-600">This email is already registered.</div>}
                  {!emailTaken && !emailChecking && email && !emailValid && (
                    <span className="text-red-600">Please enter a valid email</span>
                  )}
                  {!emailTaken && emailBlocked && <div className="text-red-600">Your email has been blocked.</div>}
                  {!emailTaken && googleMode && emailLocked && <span className="text-gray-500">Using your Google email</span>}
                </div>
              </div>
            </div>
                  ...
                  <div className="relative w-full">
                    <Listbox.Button className="relative w-full cursor-default rounded-md border border-gray-300 bg-white py-2 pl-8 pr-7 text-left text-sm focus:outline-none focus:ring-2 focus:ring-[#2AB09C]">
                      <span className="absolute inset-y-0 left-2 flex items-center">
                        <ReactCountryFlag svg countryCode={country.code} style={{ width: '16px', height: '16px', borderRadius: 2 }} />
                      </span>
                    ...
                    <Listbox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md border border-gray-200 bg-white py-1 text-sm shadow-lg focus:outline-none">
                      {COUNTRY_OPTIONS.map((c) => (
                        ...
                        >
                          <>
                            <span className="absolute inset-y-0 left-2 flex items-center">
                              <ReactCountryFlag svg countryCode={c.code} style={{ width: '16px', height: '16px', borderRadius: 2 }} />
                            </span>
              ...
              <div>
                ...
                <div className="flex">
                  ...
                  <input
                    ...
                    className={`w-full px-3 py-2.5 border rounded-r-md focus:ring-2 focus:ring-[#2AB09C] outline-none ${phoneTaken || (!validPhone && phoneDigits.length > 0) ? 'border-red-500' : 'border-gray-300'
                      }`}
                    placeholder="10-digit mobile"
                  />
                </div>
                <div className="mt-1 text-xs">
                  {phoneChecking && <span className="text-gray-500">Checking number…</span>}
                  {!validPhone && phoneDigits.length > 0 && <span className="text-red-600">Enter a valid 10-digit mobile number</span>}
                  {validPhone && phoneTaken && <span className="text-red-600">This mobile number is already used (registered or pending)</span>}
                  {validPhone && phoneBlocked && <span className="text-red-600">Your phone number has been blocked.</span>}
                </div>
              </div>
            </div>
            ...
            <p className="text-sm text-gray-600 text-center">
              ...
              <button type="button" onClick={onSwitch} className="text-[#2AB09C] font-medium">
                Login
              </button>
            </p>
          </form>
