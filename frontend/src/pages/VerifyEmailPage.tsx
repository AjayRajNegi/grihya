...
export default function VerifyEmailPage() {
  ...
  const [info, setInfo] = useState(
    email ? `We have sent a verification email to ${email}.` : 'We have sent a verification email.'
  );
  const [err, setErr] = useState('');
  const [verified, setVerified] = useState(false);
  const timerRef = useRef<number | null>(null);

  const API_BASE = import.meta.env.VITE_API_URL || 'http://backend.easylease.services/api';
    ...
    async function confirm() {
      ...
      try {
        ...
        if (kind === 'email_change') {
          ...
          try {
            localStorage.setItem('verified_event', JSON.stringify({ kind: 'email_change', ts: Date.now() }));
          } catch {}
        ...
        try {
          localStorage.setItem('verified_event', JSON.stringify({ kind: 'signup', ts: Date.now() }));
        } catch {}
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 403 || res.status === 401)
          throw new Error(json?.message || 'Link expired. Please initiate it again.');
  ...
  useEffect(() => {
    if (!tokenParam && (!email || !resendUrl)) navigate('/login', { replace: true });
      ...
      {!verified && !tokenParam && (
        <>
          <p className="text-sm text-gray-600 mb-4">
            Please click the verification link we sent to <b>{email}</b>. You can request it again after the timer ends.
          </p>
          <button
            type="button"
            disabled={cooldown > 0}
