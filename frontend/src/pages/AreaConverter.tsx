...
const REGION_PRESETS: Preset[] = [
  {
    key: "up",
    label: "UP/Uttarakhand (20 biswa = 1 bigha)",
    bighaSqft: 27000,
    kathaSqft: 1361,
    biswaPerBigha: 20,
    notes: "Values vary by district; typical working defaults.",
  },
  {
    key: "rajasthan",
    label: "Rajasthan (Pucca bigha)",
    bighaSqft: 27225,
    biswaPerBigha: 20,
    notes: "Pucca bigha; kuchha differs. Verify locally.",
  },
  {
    key: "wb",
    label: "West Bengal (20 katha = 1 bigha)",
    bighaSqft: 14400,
    kathaSqft: 720,
    notes: "Common Kolkata/WB convention.",
  },
  {
    key: "assam",
    label: "Assam (5 katha = 1 bigha)",
    bighaSqft: 14400,
    kathaSqft: 2880,
    notes: "Assam convention: 1 bigha = 5 katha.",
  },
  {
    key: "maha",
    label: "Maharashtra/Karnataka (Guntha)",
    gunthaSqft: 1089,
    notes: "1 guntha ≈ 1089 sqft; 40 guntha = 1 acre.",
  },
  {
    key: "standard",
    label: "Standard (International)",
    notes: "Use acre/hectare/sqft/sqyd without regional units.",
  },
  {
    key: "custom",
    label: "Custom (set your own)",
    bighaSqft: 27000,
    kathaSqft: 1361,
    biswaPerBigha: 20,
    gunthaSqft: 1089,
...
const AREA_UNITS: {
  ...
  label: string;
  aliases?: string[];
  group: "Global" | "India (Common)" | "India (Regional)";
}[] = [
  {
    key: "sqft",
    label: "Square Feet (sq ft)",
    aliases: ["ft²", "sq ft", "feet"],
    group: "Global",
  },
  {
    key: "sqm",
...
function getSqmPerUnit(
...
): { sqm: number; warn?: string } {
  const bighaSqft =
    (preset.key === "custom" ? custom.bighaSqft : preset.bighaSqft) ||
    undefined;
  const kathaSqft =
    (preset.key === "custom" ? custom.kathaSqft : preset.kathaSqft) ||
    undefined;
  const biswaPerBigha =
    (preset.key === "custom" ? custom.biswaPerBigha : preset.biswaPerBigha) ||
    undefined;
  const gunthaSqft =
    (preset.key === "custom" ? custom.gunthaSqft : preset.gunthaSqft) ||
    preset.gunthaSqft ||
    SQM_PER_GUNTHA_DEFAULT / SQM_PER_SQFT;
  ...
  switch (unit) {
    ...
    case "gaj":
      return {
        sqm: SQM_PER_SQYD,
        warn: "Assumed Gaj = Square Yard (regional variations exist).",
      };
    case "acre":
    ...
    case "decimal":
      return { sqm: SQM_PER_DECIMAL };
    case "guntha":
      return {
        sqm: gunthaSqft * SQM_PER_SQFT,
        warn: "Guntha assumed ≈ 1089 sqft unless overridden.",
      };
    case "bigha":
      if (!bighaSqft)
        return {
          sqm: NaN,
          warn: "Bigha not defined for this preset. Choose a regional preset or set Custom.",
        };
      return {
        sqm: bighaSqft * SQM_PER_SQFT,
        warn: "Bigha varies by region; using preset/custom value.",
      };
    case "katha":
      if (!kathaSqft)
        return {
          sqm: NaN,
          warn: "Katha not defined for this preset. Choose a regional preset or set Custom.",
        };
      return {
        sqm: kathaSqft * SQM_PER_SQFT,
        warn: "Katha varies by region; using preset/custom value.",
      };
    case "biswa": {
      if (!bighaSqft || !biswaPerBigha)
        return {
          sqm: NaN,
          warn: "Biswa not defined for this preset. Choose a regional preset or set Custom (Biswa per Bigha).",
        };
      const biswaSqft = bighaSqft / biswaPerBigha;
      return {
        sqm: biswaSqft * SQM_PER_SQFT,
        warn: "Biswa varies by region; using preset/custom value.",
      };
    }
    default:
      return { sqm: NaN };
  ...
  const faqs: FaqItem[] = [
    {
      ...
      a: (
        <p>
          These are customary units defined historically and vary by local
          revenue practice. Always check local records or your sale deed. Use
          our regional presets or set Custom to match your locality.
        </p>
      ),
    },
    {
      ...
      a: (
        <p>
          Often yes (1 Gaj ≈ 1 Sq Yard = 9 sq ft), but colloquial use can vary.
          The converter assumes Gaj = Sq Yard by default.
        </p>
      ),
    },
    {
      ...
      a: (
        <p>
          For listings, 2 - 3 decimals are common. For legal/engineering use,
          increase precision and verify unit definitions/presets.
        </p>
      ),
    },
    {
      ...
      a: (
        <p>
          This converter assumes total area is known. For irregular plots,
          compute area from dimensions (survey/map) and then convert units.
        </p>
      ),
    },
  ];
        ...
        <div className="mx-auto max-w-3xl">
          ...
          <div className="text-center mb-10">
            ...
            <p className="text-gray-600 mt-5 max-w-2xl mx-auto">
              Answers to common questions about land area units, regional
          ...
          <p className="text-xs text-slate-500 mt-4 text-center">
            Disclaimer: Conversions are illustrative. Regional units (Bigha,
            Katha, Biswa, Guntha) vary by locale; confirm with local
            authorities.
          </p>
        </div>
      </div>
...
const AreaConverter: React.FC = () => {
  ...
  const preset = useMemo(
    () => REGION_PRESETS.find((p) => p.key === presetKey)!,
    [presetKey]
  );
  ...
  const { result, warn } = useMemo(
    () => convertArea(value, from, to, preset, custom),
    [value, from, to, preset, custom]
  );

  // History
  const [history, setHistory] = useState<
    { v: number; from: AreaUnitKey; to: AreaUnitKey; res: number; ts: number }[]
  >([]);
  useEffect(() => {
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="mb-6">
          ...
          <p className="mt-2 text-slate-600">
            Convert between Sqft, Sqm, Acre, Hectare, Gaj, Bigha, Cent, Sq Yard
            and more. Supports regional units with presets (UP/UK, Rajasthan,
            West Bengal, Assam, Maharashtra) and custom overrides.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <Card
            ...
            subtitle="Swap units, set precision, and copy results"
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <label className="block">
                ...
                <select
                  value={from}
                  onChange={(e) => setFrom(e.target.value as AreaUnitKey)}
                  className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
                >
                  ...
                  <optgroup label="India (Common)">
                    {AREA_UNITS.filter((u) => u.group === "India (Common)").map(
                      (u) => (
                        <option key={u.key} value={u.key}>
                          {u.label}
                        </option>
              ...
              <label className="block">
                ...
                <select
                  value={to}
                  onChange={(e) => setTo(e.target.value as AreaUnitKey)}
                  className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
                >
                  <optgroup label="Global">
                    {AREA_UNITS.filter((u) => u.group === "Global").map((u) => (
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-[1fr_auto] gap-3 items-center">
              ...
              <div className="flex gap-2 md:justify-end">
                <button
                  onClick={copyOut}
                  className="inline-flex items-center gap-2 rounded-md border border-slate-300 px-3 py-2 text-sm text-slate-800 hover:bg-white"
                >
                  {copied ? (
            ...
            <div className="mt-5">
              ...
              <details className="mt-2">
                <summary className="text-[12px] text-emerald-700 cursor-pointer">
                  Show more
                </summary>
          ...
          <div className="space-y-4">
            <Card
              title="Regional presets (India)"
              subtitle="Set how Bigha/Katha/Biswa/Guntha are interpreted"
            >
              <label className="block">
                <span className="text-xs font-medium text-slate-700">
                  Preset
                ...
                <select
                  value={presetKey}
                  onChange={(e) =>
                    setPresetKey(e.target.value as RegionPresetKey)
                  }
                  className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
                >
              ...
              <div className="mt-3 text-[11px] text-slate-500">
                Tip: Choose a preset (e.g., West Bengal) to apply common
                regional definitions. Select Custom to override exact sizes.
              </div>
            </Card>
        ...
        {/* Table: 1 [from] equals ... */}
        <Card
          title={`Conversion table: 1 ${from} equals`}
          subtitle="Quick look across all supported units"
        >
          ...
          <div className="mt-2 text-[11px] text-slate-500">
            Note: Regional units (Bigha/Katha/Biswa/Guntha) vary by
            state/district. Use presets or Custom to match local practice.
          </div>
        </Card>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-1 gap-6 mt-8">
          <Card
            title="Other Units"
            subtitle="Handy length and volume converters"
          >
          ...
          <Card
            title="What is a Land Area Calculator?"
            className="lg:col-span-2"
          >
            <p className="text-sm text-slate-700">
              A land area calculator converts between common and regional land
              units. It helps buyers, sellers, and developers quickly translate
              plot sizes from, say, Gaj or Bigha into Sqft/Acre, or Hectare into
              Sqm. Because Indian regional units like Bigha, Katha, Biswa, and
              Guntha vary by state/district, this tool lets you choose a
              regional preset or set custom values.
            </p>
            <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-slate-700">
              <div className="rounded border border-slate-200 p-3">
                ...
                <p className="mt-1">
                  For large plots, small rounding differences can compound into
                  material differences in valuation and circle rates.
                </p>
              </div>
              <div className="rounded border border-slate-200 p-3">
                <div className="font-medium text-slate-900">
                ...
                <p className="mt-1">
                  Verify local practice (revenue records, sale deed). Use Custom
                  if your locality uses a different standard.
                </p>
              </div>
            </div>
          </Card>
          ...
          <Card title="Most Searched Conversion Units for Land Area Calculator">
            <ol className="list-decimal pl-5 text-sm text-slate-700 space-y-2">
              <li>
                ...
                - Area of a square with 1‑ft sides. Widely used in India and
              ...
              <li>
                ...
                - International SI unit (m²). 1 sq m = 10.7639 sq ft.
              </li>
              <li>
                <span className="font-medium text-slate-900">Hectare (ha)</span>{" "}
                - Metric land unit. 1 ha = 10,000 sq m ≈ 2.47105 acres.
              </li>
              <li>
                <span className="font-medium text-slate-900">Acre</span> -
                Imperial unit. 1 acre = 43,560 sq ft ≈ 4046.8564 sq m.
              </li>
              <li>
                <span className="font-medium text-slate-900">
                  Bigha (regional)
                </span>{" "}
                - Traditional unit; size varies by state (e.g., ~14,400 sq ft in
                WB; ~27,000 - 27,225 sq ft in UP/Rajasthan). Always verify
                locally.
              </li>
            </ol>
          </Card>
