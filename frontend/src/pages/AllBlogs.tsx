...
function readingTimeFromBlocks(blocks?: Block[]) {
  const text = (blocks || [])
    .map((b) => {
      ...
      if (b.type === "problem_solution")
        return `${(b as any).data?.problem || ""} ${
          (b as any).data?.solution || ""
        }`;
      return "";
    })
    .join(" ");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await apiGet<Post[] | ApiPaginated<Post>>(
          `/posts?${qs.toString()}`
        );
        if (!mounted) return;
            ...
            <div className="mb-6 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-slate-600">
                  Insights, tips and guides to help you make smarter decisions.
                </p>
              </div>
            </div>
