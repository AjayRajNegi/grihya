...
function computeScenario(i: Inputs) {
  ...
  const upfrontBuy =
    ((i.stampDutyPct + i.registrationPct + i.otherFeesPct) / 100) * price;
  ...
  const buyMonthlySavingsFV = fvAnnuity(
    monthlyDiffIfBuyCheaper,
    i.investReturnPct,
    M
  );
  ...
  const rentMonthlyInvestFV = fvAnnuity(
    monthlyDiffIfRentCheaper,
    i.investReturnPct,
    M
  );
...
const PresetChips: React.FC<{ onApply: (p: Partial<Inputs>) => void }> = ({
  onApply,
}) => {
  return (
  ...
  const faqs: FaqItem[] = [
    {
      ...
      a: (
        <p>
          Not explicitly. You can approximate by lowering effective EMI outflow
          or adding a small annual cash‑back to the rent scenario. We’ll add a
          toggle soon.
        </p>
      ),
    },
    {
      ...
      a: (
        <p>
          Both exist. We model it as % of property value for simplicity. You can
          adjust the % to match your building.
        </p>
      ),
    },
    {
      ...
      a: (
        <p>
          Renting often wins on short horizons due to lower friction. Use a 3 -
          5 year horizon to see this effect.
        </p>
      ),
    },
    {
      ...
      a: (
        <p>
          We default to 5% p.a. It varies by locality and cycle. Adjust it to
          your market.
        </p>
      ),
    },
        ...
        <div className="mx-auto max-w-3xl">
          ...
          <div className="text-center mb-10">
            ...
            <p className="text-gray-600 mt-5 max-w-2xl mx-auto">
              Answers to common questions about comparing home buying and
              renting.
            </p>
          </div>
          ...
          <p className="text-xs text-slate-500 mt-4 text-center">
            Disclaimer: This tool is for education only. Actual decisions should
            consider detailed costs, taxes, liquidity, and your risk tolerance.
          </p>
        </div>
      </div>
    </section>
...
const BuyVsRent: React.FC = () => {
  ...
  const breakEven = useMemo(
    () => findBreakEven(inp, Math.max(5, Math.min(35, inp.loanYears))),
    [inp]
  );
  const navigate = useNavigate();

  const applyPreset = (p: Partial<Inputs>) =>
    setInp((prev) => ({ ...prev, ...p }));
  ...
  const rec =
    scenario.diff > 0
      ? {
          text: "Buying is better than renting in this setup.",
          tone: "pos" as const,
        }
      : scenario.diff < 0
      ? {
          text: "Renting is better than buying in this setup.",
          tone: "neg" as const,
        }
      : {
          text: "Both look roughly similar with current assumptions.",
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="mb-6">
          ...
          <p className="mt-2 text-slate-600">
            Use the calculator to compare monthly outflows and net worth over
            time. Adjust assumptions for appreciation, rent growth, loan rates,
        ...
        <section className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 mb-6">
          <Card title="Property & financing">
            <div className="grid grid-cols-2 gap-3">
              <NumberField
                label="Home price"
                value={inp.homePrice}
                onChange={(v) => setInp({ ...inp, homePrice: v })}
                suffix="₹"
              />
              <NumberField
                label="Down payment"
                value={inp.downPct}
                onChange={(v) => setInp({ ...inp, downPct: v })}
                suffix="%"
              />
              <NumberField
                label="Loan rate (p.a.)"
                value={inp.loanRate}
                onChange={(v) => setInp({ ...inp, loanRate: v })}
                suffix="%"
              />
              <NumberField
                label="Loan tenure"
                value={inp.loanYears}
                onChange={(v) => setInp({ ...inp, loanYears: v })}
                suffix="years"
              />
              <NumberField
                label="Stamp duty"
                value={inp.stampDutyPct}
                onChange={(v) => setInp({ ...inp, stampDutyPct: v })}
                suffix="%"
              />
              <NumberField
                label="Registration"
                value={inp.registrationPct}
                onChange={(v) => setInp({ ...inp, registrationPct: v })}
                suffix="%"
              />
              <NumberField
                label="Other fees"
                value={inp.otherFeesPct}
                onChange={(v) => setInp({ ...inp, otherFeesPct: v })}
                suffix="%"
              />
              <NumberField
                label="Selling costs"
                value={inp.sellCostPct}
                onChange={(v) => setInp({ ...inp, sellCostPct: v })}
                suffix="%"
              />
              <NumberField
                label="Maintenance (p.a.)"
                value={inp.maintPct}
                onChange={(v) => setInp({ ...inp, maintPct: v })}
                suffix="% of price"
              />
              <NumberField
                label="Property tax (p.a.)"
                value={inp.propTaxPct}
                onChange={(v) => setInp({ ...inp, propTaxPct: v })}
                suffix="% of price"
              />
              <NumberField
                label="Appreciation"
                value={inp.appreciationPct}
                onChange={(v) => setInp({ ...inp, appreciationPct: v })}
                suffix="% p.a."
              />
            </div>
          </Card>
          ...
          <Card title="Rent & investing">
            <div className="grid grid-cols-2 gap-3">
              <NumberField
                ...
                value={inp.rentNow}
                onChange={(v) => setInp({ ...inp, rentNow: v })}
                suffix="₹/mo"
              />
              <NumberField
                label="Rent escalation"
                value={inp.rentEscPct}
                onChange={(v) => setInp({ ...inp, rentEscPct: v })}
                suffix="% p.a."
              />
              <NumberField
                label="Investment return"
                value={inp.investReturnPct}
                onChange={(v) => setInp({ ...inp, investReturnPct: v })}
                suffix="% p.a."
              />
              <NumberField
                label="Deposit"
                value={inp.depositMonths}
                onChange={(v) => setInp({ ...inp, depositMonths: v })}
                suffix="months"
              />
              <NumberField
                label="Decision horizon"
                value={inp.horizonYears}
                onChange={(v) => setInp({ ...inp, horizonYears: v })}
                suffix="years"
              />
            </div>
          </Card>
          ...
          <Card title="Quick math" subtitle="Key numbers based on your inputs">
            <div className="grid grid-cols-2 gap-3">
              ...
              <Kpi
                label="Avg rent (monthly)"
                value={fmtINR(scenario.avgRent)}
              />
              <Kpi
                label="Maint + Tax (monthly)"
                value={fmtINR(scenario.monthlyMaint + scenario.monthlyPropTax)}
              />
              <Kpi
                label="Buy monthly outflow"
                value={fmtINR(scenario.monthlyBuyOutflow)}
              />
              <Kpi
                label="Upfront costs"
                value={fmtINR(scenario.upfrontBuy)}
                hint="Stamp + reg + other"
              />
        ...
        <section className="mb-8 grid grid-cols-1 gap-4 lg:grid-cols-3">
          <Card title="Outcome after your horizon">
            <div className="grid grid-cols-1 gap-3">
              <Kpi
                ...
                value={fmtINR(scenario.buyWealth)}
              />
              <Kpi
                label="Rent scenario net worth"
                value={fmtINR(scenario.rentWealth)}
              />
              <Kpi
                label="Difference (Buy - Rent)"
                value={`${scenario.diff >= 0 ? "+" : ""}${fmtINR(
                  scenario.diff
                )}`}
                tone={
                  scenario.diff > 0
          ...
          <Card title="Break-even and equity context">
            <div className="grid grid-cols-1 gap-3">
              <Kpi
                label="Estimated break-even year"
                value={breakEven ? `${breakEven} years` : "Not within horizon"}
              />
              <Kpi
                label="Future property value"
                value={fmtINR(scenario.propFV)}
              />
              <Kpi
                label="Loan outstanding at horizon"
                value={fmtINR(scenario.outstanding)}
              />
              <Kpi
                label="Selling costs at horizon"
                value={fmtINR(scenario.sellCost)}
              />
            </div>
          </Card>
          ...
          <Card title="What this means">
            <ul className="list-disc pl-5 text-sm text-slate-700 space-y-1.5">
              <li>
                We compare net worth: equity in a home (after selling costs and
                outstanding loan) vs. investing your down-payment + monthly
                savings when renting.
              </li>
              <li>
                Rent escalates yearly; EMI is fixed; maintenance and property
                tax are estimated from property value.
              </li>
              <li>
                Investment returns are compounded monthly for both initial lump
                sum and monthly contributions.
              </li>
            </ul>
          </Card>
        </section>
        ...
        <section className="grid grid-cols-1 gap-6 md:grid-cols-2 mb-8">
          <Card title="Buying  -  Good fit when">
            <ul className="text-slate-700 text-sm space-y-1.5">
              ...
              <li>
                • You can afford EMI + maintenance comfortably with buffer.
              </li>
              <li>• Schools, commute, and neighborhood fit long-term needs.</li>
              <li>
                • You value asset-building and potential price appreciation.
              </li>
            </ul>
          </Card>
          <Card title="Renting  -  Good fit when">
            <ul className="text-slate-700 text-sm space-y-1.5">
              <li>
                • You value flexibility (job/city changes) or shorter horizon (≤
                3 - 5 years).
              </li>
        ...
        <section className="grid grid-cols-1 gap-6 md:grid-cols-2 mb-8">
          <Card title="Simple rules of thumb">
            <ul className="text-slate-700 text-sm space-y-1.5">
              <li>
                • If annual rent is ≪ 2 - 3% of the home price, renting often
                makes sense short-term.
              </li>
              <li>
                • If you’ll stay long-term and can afford comfortably, buying
                often makes sense.
              </li>
              <li>
                • Always budget for maintenance, taxes, furnishings, and move-in
                costs.
              </li>
              <li>
                • Liquidity matters: don’t exhaust your emergency fund to buy.
              </li>
            </ul>
          </Card>
          <Card title="Methodology (how we compare)">
            <ul className="text-slate-700 text-sm space-y-1.5">
              <li>
                • Buy: equity = future value of property − loan outstanding −
                selling costs + FV of monthly savings if EMI+maint &lt; rent.
              </li>
              <li>
                • Rent: wealth = FV of down payment + upfront costs (invested) −
                deposit + FV of monthly savings (if rent &lt; buy) + deposit
                returned at end.
              </li>
              <li>
                • Rent escalates annually; investment return compounded monthly;
                EMI fixed; maintenance/tax based on % of property value.
              </li>
              <li>
                • Results are illustrative - tax benefits, insurance, vacancy,
                and transaction timing can change outcomes.
              </li>
            </ul>
          </Card>
        </section>
