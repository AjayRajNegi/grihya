...
const Agents: React.FC = () => {
    ...
    const [contacts, setContacts] = useState<Record<string, { phone?: string | null; email?: string | null; loading?: boolean; error?: string | null }>>({});

    async function fetchContact(agentId: string | number) {
        setContacts((prev) => ({ ...prev, [String(agentId)]: { ...(prev[String(agentId)] || {}), loading: true, error: null } }));
        ...
        try {
            ...
            if (res.status === 401) {
                setContacts((prev) => ({ ...prev, [String(agentId)]: { ...(prev[String(agentId)] || {}), loading: false, error: 'Login required' } }));
                return;
            }
            if (!res.ok) {
            ...
            setContacts((prev) => ({ ...prev, [String(agentId)]: { phone: data.phone ?? null, email: data.email ?? null, loading: false, error: null } }));
        } catch (e: any) {
            setContacts((prev) => ({ ...prev, [String(agentId)]: { ...(prev[String(agentId)] || {}), loading: false, error: e.message || 'Error' } }));
        }
    }
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between mb-6">
                    <div>
                        <div className="mb-6 flex items-center gap-3">
                            ...
                            <div className=''>
                                <h1 className="text-2xl md:text-3xl font-bold text-slate-900">Property Agents</h1>
                            </div>
                        </div>
                                <p className="text-slate-600 mt-1">Showing all brokers{total ? ` · ${total} ` : ''}</p>
                    </div>
                ...
                {loading ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {Array.from({ length: 6 }).map((_, i) => (
                            <div key={i} className="rounded-xl bg-white border border-slate-200 p-5 shadow-sm h-40 animate-pulse" />
                        ))}
                    </div>
                ) : filtered.length === 0 ? (
                    ...
                    <>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {filtered.map((a) => {
                                ...
                                return (
                                    <div key={a.id} className="rounded-xl bg-white border border-slate-200 p-5 shadow-sm hover:shadow-md transition">
                                        ...
                                        <div className="mt-4 flex flex-col gap-2">
                                            ...
                                            {!show ? (
                                                <button
                                                    ...
                                                    onClick={async () => {
                                                        if (isAuthed) {
                                                            setReveal((r) => ({ ...r, [String(a.id)]: true }));
                                                    ...
                                                    title={isAuthed ? 'View contact details' : 'Login to see'}
                                                ...
                                                <div className="rounded-md border border-emerald-200 p-3 text-sm space-y-1.5">
                                                    {contacts[String(a.id)]?.loading ? (
                                                        <div className="text-slate-600">Loading contact…</div>
                                                    ) : contacts[String(a.id)]?.error ? (
                                                        <div className="text-red-600">{contacts[String(a.id)]?.error}</div>
                                                    ) : (
                                                        <>
                                                            <div className="flex items-center gap-2 text-slate-700">
                                                                <PhoneIcon className="h-4 w-4" />
                                                                <a href={contacts[String(a.id)]?.phone ? `tel:${contacts[String(a.id)]?.phone}` : '#'} className="font-medium text-[#2AB09C]">
                                                                    {contacts[String(a.id)]?.phone || '—'}
                                                                </a>
                                                            </div>
                                                            <div className="flex items-center gap-2 text-slate-700">
                                                                <MailIcon className="h-4 w-4" />
                                                                <a href={contacts[String(a.id)]?.email ? `mailto:${contacts[String(a.id)]?.email}` : '#'} className="font-medium text-[#2AB09C]">
