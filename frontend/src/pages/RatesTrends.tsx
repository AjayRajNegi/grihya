...
function generateCitySeries(city: City, months: number): CitySeries {
  ...
  const seasonality = (i: number) =>
    1 + 0.04 * Math.sin((2 * Math.PI * i) / 12); // ±4%
  ...
  return {
    medianPsqft: {
      name: `${city} • Median Price/sq.ft`,
      color: "#0ea5e9",
      points: medianPsqftPts,
    },
    rentIndex: {
      name: `${city} • Rent Index`,
      color: "#10b981",
      points: rentIndexPts,
    },
    transactions: {
      name: `${city} • Transactions`,
      color: "#f59e0b",
      points: transactionsPts,
    },
    inventory: {
      name: `${city} • Inventory`,
      color: "#6366f1",
...
const LineChart: React.FC<{
  ...
  const allPoints = useMemo(
    () =>
      series
        .map((s) => s.points)
        .reduce<Point[]>((acc, arr) => acc.concat(arr), []),
    [series]
  );
  ...
  const hoverDate =
    hoverIdx != null
      ? series[0]?.points[hoverIdx]?.x.toLocaleDateString("en-IN", {
          year: "numeric",
          month: "short",
        })
      : null;
    ...
    <div ref={ref}>
      ...
      >
        ...
        {series[0]?.points.map((p, i) => {
          ...
          const label = p.x.toLocaleDateString("en-IN", {
            year: n > 20 ? "2-digit" : undefined,
            month: "short",
          });
          return (
            <text
  ...
  const faqs: FaqItem[] = [
    {
      ...
      a: (
        <p>
          Usually yes. YoY comparisons neutralize seasonal patterns (festive
          periods, academic cycles), while MoM can be noisy.
        </p>
      ),
    },
    {
      ...
      a: (
        <p>
          It approximates demand: monthly sales ÷ active inventory. Higher
          absorption implies tighter markets and potentially faster price moves.
        </p>
      ),
    },
    {
      ...
      a: (
        <p>
          Medians reduce the effect of extreme outliers (ultra‑luxury or
          distressed sales) and better represent the typical property.
        </p>
      ),
    },
    {
      ...
      a: (
        <p>
          They show relative differences; actual prices vary by micro‑pocket,
          project, floor, and amenities. Always verify with comps.
        </p>
      ),
    },
  ];
        ...
        <div className="mx-auto max-w-3xl">
          ...
          <div className="text-center mb-10">
            ...
            <p className="text-gray-600 mt-5 max-w-2xl mx-auto">
              A quick primer on reading price trends, rent growth, and market
              strength.
            </p>
          </div>
...
const RatesTrends: React.FC = () => {
  ...
  const primary = useMemo(
    () => generateCitySeries(city, months),
    [city, months]
  );
  const secondary = useMemo(
    () =>
      compareCity === "None"
        ? undefined
        : generateCitySeries(compareCity, months as number),
    [compareCity, months]
  );
  ...
  const chartSeries: Series[] = useMemo(() => {
    if (metric === "price") {
      const arr = [
        {
          ...primary.medianPsqft,
          name: `${city} • Median ₹/sq.ft`,
          color: "#0ea5e9",
        },
      ];
      if (secondary)
        arr.push({
          ...secondary.medianPsqft,
          name: `${secondary.medianPsqft.name}`,
          color: "#64748b",
        });
      return arr;
    }
    ...
    if (secondary)
      arr.push({
        ...
        name: `${secondary.transactions.name}`,
        color: "#fb923c",
      });
    return arr;
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="mb-8">
          <div className="mb-6 flex items-center gap-2">
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-slate-900">
              Real Estate Rates & Trends
            </h1>
          </div>
          <p className="mt-2 text-slate-600">
            Explore prices, rent growth, volumes and more. Compare cities, track
            YoY changes, and understand market cycles.
          </p>
        </div>
        ...
        <section className="mb-10">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div
                    className={`text-xs font-medium ${
                      l.yoy >= 0 ? "text-emerald-700" : "text-rose-700"
                    }`}
                  >
                    {percent(l.yoy)}
                  </div>
                ...
                <div className="mt-3 h-2 w-full rounded bg-slate-100">
                  <div
                    className={`h-2 rounded ${
                      l.yoy >= 0 ? "bg-emerald-500" : "bg-rose-500"
                    }`}
          ...
          <p className="mt-2 text-xs text-slate-500">
            Locality snapshots approximate relative differences; actual prices
            vary by building, floor, and age.
          </p>
        </section>
        ...
        <section className="mb-10 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-1 text-sm text-slate-600">
            For a representative loan of ₹ 50 Lac over 20 years at 8.5% p.a.,
            the approximate EMI is:
          </p>
          ...
          <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
            ...
            <AffordLine
              label="If down payment +10%"
              note="Loan ₹45L"
              value={emi(45_00_000, 8.5, 20)}
            />
          </div>
          <p className="mt-2 text-xs text-slate-500">
            EMIs are approximate; check with your lender for exact quotes.
          </p>
        </section>
...
const AffordLine: React.FC<{ label: string; value: number; note?: string }> = ({
  label,
  value,
  note,
}) => (
