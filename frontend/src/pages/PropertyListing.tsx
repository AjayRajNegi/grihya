...
const PropertyListing: React.FC = () => {
  ...
  useEffect(() => {
    ...
    async function fetchProperties() {
      ...
      try {
        ...
        if (!hasCoords && filters.location) qs.set('location', filters.location);
        ...
        if (filters.ready_to_move) qs.set('ready_to_move', String(filters.ready_to_move));
        if (filters.listed_by) qs.set('listed_by', filters.listed_by);
        if (filters.user_id) qs.set('user_id', String(filters.user_id));
        if (filters.preferred_tenants) qs.set('preferred_tenants', filters.preferred_tenants);
        if (filters.available_immediately) qs.set('available_immediately', String(filters.available_immediately));
        ...
        if (filters.priceRange) {
          ...
          if (normalized.includes('-')) {
            const [min, max] = normalized.split('-').map((n) => parseInt(n, 10));
        ...
        if (filters.bedrooms === '4+') mapped = mapped.filter((p) => (p.bedrooms ?? 0) >= 4);
        if (filters.bathrooms === '3+') mapped = mapped.filter((p) => (p.bathrooms ?? 0) >= 3);

        // Client-side fallback sorting
        if (sortBy === 'newest') {
          mapped.sort((a, b) => new Date(b.listedDate).getTime() - new Date(a.listedDate).getTime());
        } else if (sortBy === 'oldest') {
          mapped.sort((a, b) => new Date(a.listedDate).getTime() - new Date(b.listedDate).getTime());
  ...
  const handleFilterChange = (f: any) => {
    ...
    if (Array.isArray(f.amenities) && f.amenities.length) params.amenities = f.amenities;
    ...
    if (!(params as any).type && current.type) (params as any).type = current.type;

    if (current.ready_to_move) (params as any).ready_to_move = current.ready_to_move;
    if (current.listed_by) (params as any).listed_by = current.listed_by;
    if ((current as any).minPrice) (params as any).minPrice = (current as any).minPrice;
    if ((current as any).maxPrice) (params as any).maxPrice = (current as any).maxPrice;
    if ((current as any).user_id) (params as any).user_id = (current as any).user_id;
    if ((current as any).preferred_tenants) (params as any).preferred_tenants = (current as any).preferred_tenants;
    if ((current as any).available_immediately) (params as any).available_immediately = (current as any).available_immediately;
            ...
            <div className="bg-white p-4 rounded-lg shadow-sm mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <div className="flex items-center space-x-2">
                ...
                <button
                  ...
                  className={`p-2 rounded-md ${viewMode === 'grid' ? 'bg-[#CCF0E1FF] text-[#2AB09C]' : 'text-gray-500 hover:bg-gray-100'}`}
                >
                  <GridIcon className="h-5 w-5" />
                </button>
                <button
                  onClick={() => setViewMode('list')}
                  className={`p-2 rounded-md ${viewMode === 'list' ? 'bg-[#CCF0E1FF] text-[#2AB09C]' : 'text-gray-500 hover:bg-gray-100'}`}
                >
            ...
            {loading && (
              <div className={viewMode === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6' : 'space-y-6'}>
                {Array.from({ length: 6 }).map((_, i) => (
                  <div key={i} className="bg-white rounded-lg shadow-sm animate-pulse h-64" />
                ))}
              </div>
            )}
            ...
            {!loading && properties.length > 0 && (
              viewMode === 'grid' ? (
                ...
                <div className="space-y-6">
                  {properties.map((property) => (
                    <div key={property.id} className="bg-white rounded-lg shadow-sm overflow-hidden flex flex-col sm:flex-row">
                      ...
                      <div className="p-4 sm:p-6 flex-1">
                        ...
                        <p className="text-[#2AB09C] font-bold text-lg mb-2">
                          ₹{property.price.toLocaleString()}{property.for === 'rent' ? '/month' : ''}
                        </p>
