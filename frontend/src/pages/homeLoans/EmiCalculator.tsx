...
const PaymentDonut: React.FC<{ principal: number; interest: number }> = ({
  principal,
  interest,
}) => {
  ...
  return (
    <div className="flex items-center gap-4">
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        >
          {total
            ? `${Math.round(pPct * 100)}% P / ${Math.round(iPct * 100)}% I`
            : " - "}
        </text>
      </svg>
...
const EMICalculator: React.FC = () => {
  ...
  const [selectedCalYear, setSelectedCalYear] =
    useState<number>(defaultStartYear);

  const navigate = useNavigate();

  const schedule = useMemo(
    () => buildSchedule(amount, rate, years),
    [amount, rate, years],
  );
  ...
  const scheduleCal: RowCal[] = useMemo(() => {
    ...
    for (let i = 0; i < schedule.length; i++) {
      const r = schedule[i];
      const { year: cy, month: cm } = addMonthsToYearMonth(
        startYear,
        startMonth,
        i + 1,
      );
  ...
  const monthlyEmi = useMemo(
    () => (schedule.length ? schedule[0].emi : 0),
    [schedule],
  );
  ...
  const calYears = useMemo(() => {
    ...
    for (let i = 0; i < scheduleCal.length; i++)
      set.add(scheduleCal[i].calYear);
  ...
  const faqs: FaqItem[] = [
    {
      ...
      a: (
        <p>
          It’s a tool that computes your monthly EMI based on three inputs: loan
          amount, interest rate (p.a.), and loan tenure. It also shows the
          amortization schedule - how much of each EMI goes to interest vs
          principal, and how the outstanding balance drops over time.
        </p>
      ),
    },
    {
      ...
      a: (
        <p>
          EMI (Equated Monthly Instalment) is a fixed amount you pay each month
          to the lender. It includes both interest and principal. Early EMIs are
          interest-heavy; over time, the principal portion increases.
        </p>
      ),
    },
    {
      ...
      a: (
        <ul className="list-disc space-y-1 pl-5">
          <li>
            Enter loan amount (₹), interest rate (p.a.), and tenure (years).
          </li>
          <li>
            Select your loan start Year and Month; schedule begins next month.
          </li>
          <li>
            Review EMI and total interest outgo; explore the month-wise schedule
            by calendar year.
          </li>
          <li>
            Use the illustration to test sensitivity to rate, tenure, and
            amount.
          </li>
        </ul>
      ),
    ...
    },
    {
      ...
      a: (
        <ul className="list-disc space-y-1 pl-5">
          <li>
            Choose a longer tenure (reduces EMI, increases total interest).
          </li>
      ...
      <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <div className="mb-6">
          ...
          <p className="mt-2 text-slate-600">
            Calculate your monthly EMI, view the month‑wise schedule, and
            explore how changes in rate, tenure, and amount affect your
            payments.
          </p>
        </div>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              ...
              <label className="block">
                ...
                <input
                  ...
                  value={years}
                  onChange={(e) =>
                    setYears(parseInt(e.target.value || "1", 10))
                  }
                />
              </label>
            </div>
            ...
            <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
              <label className="block">
                ...
                <select
                  ...
                  value={startMonth}
                  onChange={(e) =>
                    setStartMonth(parseInt(e.target.value || "1", 10))
                  }
                >
                  {MONTHS.map((m, i) => (
              ...
              <label className="block">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setStartYear(
                      parseInt(e.target.value || String(startYear), 10),
                    )
                  }
                >
                  {Array.from(
                    { length: 41 },
                    (_, k) => k + new Date().getFullYear(),
                  ) // current to +40
                    .map((y) => (
          ...
          <Card title="Quick summary">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-1">
              ...
              <Kpi
                ...
                value={fmtINR(totals.totalInterest)}
              />
              <Kpi
                label="Principal (Total)"
                value={fmtINR(totals.totalPrincipal)}
              />
            </div>
          </Card>
        </div>
        ...
        <Card
          ...
          subtitle="Month-by-month breakup of interest and principal (calendar view)"
        >
          <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-xs text-slate-600">
              Tenure: {years} years ({years * 12} months). First EMI:{" "}
              {MONTHS[scheduleCal[0]?.calMonth - 1] || " - "}{" "}
              {scheduleCal[0]?.calYear || " - "}.
            </div>
            <div className="flex items-center gap-2">
              <label className="text-sm text-slate-700">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setSelectedCalYear(parseInt(e.target.value || "0", 10))
                  }
              ...
              <button
                ...
                onClick={() =>
                  downloadCSV(
                    "emi_schedule_all.csv",
                    toCSVWithCalendar(scheduleCal),
                  )
                }
              ...
              <button
                ...
                onClick={() =>
                  downloadCSV(
                    `emi_schedule_${selectedCalYear}.csv`,
                    toCSVWithCalendar(filteredRows),
                  )
                }
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-2">
          <Card title="An illustration: how EMI changes with key factors">
            ...
            <p className="mt-3 text-xs text-slate-500">
              Note: Increasing tenure lowers EMI but increases total interest
              paid across the loan.
            </p>
          </Card>
          ...
          <Card title="How the Home Loan EMI Calculator helps you">
            <ul className="list-disc space-y-1.5 pl-5 text-sm text-slate-700">
              <li>
                Plan monthly cashflows and affordability before you apply.
              </li>
              <li>
                Compare lender offers quickly by adjusting rate and tenure.
              </li>
              <li>
                Understand interest vs principal over time to plan prepayments.
              </li>
            ...
            <div className="mt-4">
              ...
              <ul className="mt-1 list-disc space-y-1.5 pl-5 text-sm text-slate-700">
                <li>
                  Choose a longer tenure (trade-off: higher overall interest).
                </li>
                <li>
                  Increase down payment or make periodic part-prepayments.
                </li>
                <li>
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-2">
          <Card title="What is Home Loan EMI Calculator?">
            <p className="text-sm text-slate-700">
              A Home Loan EMI Calculator estimates your monthly instalment based
              on loan amount, interest rate, and tenure. It also produces a
              detailed amortization schedule showing interest and principal
              split every month.
            </p>
          </Card>
          <Card title="What is Home Loan EMI?">
            <p className="text-sm text-slate-700">
              EMI stands for Equated Monthly Instalment - a fixed payment you
              make each month. Each EMI comprises an interest portion and a
              principal portion. Early EMIs are interest-heavy and later EMIs
              become principal-heavy.
            </p>
          </Card>
          <Card title="How to use the EMI Calculator?">
            <ul className="list-disc space-y-1.5 pl-5 text-sm text-slate-700">
              ...
              <li>
                Select your loan start Year and Month; schedule begins next
                month.
              </li>
              <li>
                Filter the schedule by calendar year; export all or per-year
                CSV.
              </li>
            </ul>
          </Card>
          <Card title="An Illustration">
            <p className="text-sm text-slate-700">
              The EMI depends primarily on rate, tenure, and amount. Check the
              illustration to see how your EMI responds to changes.
            </p>
          </Card>
        </div>
        ...
        <p className="text-xs text-slate-500">
          Disclaimer: Calculations are indicative and may vary by lender. Always
