...
import { HelpCircle, CheckCircle2, ChevronDown, ChevronLeft, ChevronRight } from 'lucide-react';
...
// -------------------- UI atoms --------------------
const Badge: React.FC<{ children: React.ReactNode; color?: 'green' | 'gray' | 'blue' }> = ({ children, color = 'gray' }) => {
...
const RateRangeBar: React.FC<{ min: number; max: number; globalMin: number; globalMax: number }> = ({ min, max, globalMin, globalMax }) => {
  const toPct = (v: number) => ((v - globalMin) / Math.max(0.0001, (globalMax - globalMin))) * 100;
  ...
  return (
    <div>
      ...
      <div className="mt-1 text-[11px] text-slate-600">{min.toFixed(2)}% – {max.toFixed(2)}%</div>
    </div>
  );
};
...
const FAQ: React.FC<{ items: FaqItem[]; idPrefix?: string }> = ({ items, idPrefix = 'partner' }) => {
    ...
    <section className="py-12 bg-white rounded-xl border border-slate-200 px-4 sm:px-6 mt-8">
      <div className="mx-auto max-w-3xl">
        ...
        <p className="mt-4 text-xs text-slate-500">
          Disclaimer: Content is illustrative. Please confirm the latest policies/charges with the lender before applying.
        </p>
      </div>
    </section>
  );
...
type BankPreview = { slug: string; name: string; range: string; minRate: number; maxRate: number };

const CtaCarousel: React.FC<{ items: BankPreview[]; partners: Record<string, PartnerInfo> }> = ({ items, partners }) => {
  const [idx, setIdx] = useState(0);
  const [paused, setPaused] = useState(false);
  const safeItems = items.length ? items : Object.keys(partners).map(slug => ({ slug, name: partners[slug].name, range: partners[slug].range, minRate: partners[slug].minRate, maxRate: partners[slug].maxRate }));
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => setIdx((i) => (i + 1) % safeItems.length), 3000);
  ...
  return (
    ...
    >
      <div className={`bg-gradient-to-br ${gradient} p-4 sm:p-5 text-white min-h-[160px] flex flex-col justify-between`}>
        <div className="flex items-center justify-between">
          <div className="text-sm font-semibold opacity-90">Partner spotlight</div>
          <div className="text-[11px] bg-white/15 px-2 py-0.5 rounded-full">{clampedIdx + 1} / {safeItems.length}</div>
        </div>

        <div className="mt-2">
          <div className="text-xl font-bold leading-tight text-center">{partners[active.slug]?.name || active.name}</div>
          <div className="text-xs opacity-90 text-center">{tagline}</div>
          <div className="mt-2 text-sm text-center">
            Indicative rates: <span className="font-semibold">{partners[active.slug]?.range || active.range}</span>
          </div>
        </div>
          ...
          <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1.5">
            {safeItems.map((_, i) => (
              <button
                ...
                onClick={() => setIdx(i)}
                className={['h-1.5 rounded-full transition-all', clampedIdx === i ? 'w-5 bg-white' : 'w-2 bg-white/60 hover:bg-white/80'].join(' ')}
              />
            ))}
          </div>
        </>
...
const CompareOffers: React.FC<{
  ...
  const sorted = useMemo(() => {
    ...
    const cmp = (a: typeof rows[number], b: typeof rows[number]) => {
      ...
      if (sortKey === 'ltv') return (parseLtv(a.maxLtv) - parseLtv(b.maxLtv)) * dir;
  ...
  return (
    <Card
      ...
      subtitle={`Sorted by ${sortKey === 'emi' ? 'EMI' : sortKey === 'rate' ? 'Typical rate' : sortKey === 'ltv' ? 'Max LTV' : 'Name'} (${sortDir}) • EMI based on ₹${(sampleAmount / 1e5).toFixed(1)}L for ${sampleYears} yrs`}
      className="mt-2"
    >
      {/* Controls */}
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 text-sm ${active ? 'bg-emerald-600 text-white' : 'bg-white text-slate-700 hover:bg-slate-50'}`}
                >
                  {label}
                </button>
              );
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {sorted.map((r) => {
            ...
            return (
              <div
                key={r.slug}
                className={`rounded-xl border p-4 hover:shadow-sm transition ${isActive ? 'border-emerald-300 bg-emerald-50/40' : 'border-slate-200 bg-white'}`}
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="font-semibold text-slate-900">{r.name}</div>
                    <div className="text-[11px] text-slate-500 mt-0.5">Typical: {r.range}</div>
                  </div>
                  {isActive && <Badge color="green">You’re here</Badge>}
                </div>

                <div className="mt-3">
                  <div className="text-xs font-medium text-slate-700 mb-1">Interest band vs market</div>
                  <RateRangeBar min={r.minRate} max={r.maxRate} globalMin={globalMin} globalMax={globalMax} />
                </div>

                <div className="mt-3 flex items-center justify-between">
                  <div>
                    <div className="text-xs text-slate-600">Est. EMI</div>
                    <div className="text-lg font-semibold">{fmtINR(r.emi)}/mo</div>
                  </div>
                  <Link
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {sorted.map((r) => {
                const isActive = r.slug === selectedSlug;
                return (
                  <tr key={r.slug} className={`odd:bg-white even:bg-slate-50 ${isActive ? 'ring-1 ring-emerald-200 bg-emerald-50/40' : ''}`}>
                    <td className="px-3 py-2">
                      <div className="font-medium text-slate-900">{r.name}</div>
                      <div className="text-[11px] text-slate-500">Processing: {r.processingFee}</div>
                      <div className="text-[11px] text-slate-500">Prepayment: {r.prepayment}</div>
                    </td>
                    <td className="px-3 py-2 w-[260px] align-top">
                      <RateRangeBar min={r.minRate} max={r.maxRate} globalMin={globalMin} globalMax={globalMax} />
                    </td>
                    <td className="px-3 py-2 align-top">{r.maxLtv}</td>
                    <td className="px-3 py-2 text-right font-semibold align-top">{fmtINR(r.emi)}</td>
      ...
      <div className="text-[11px] text-slate-500 mt-3">Note: Policies/fees can change; confirm with the lender.</div>
    </Card>
  );
};
...
const PartnerDetail: React.FC = () => {
  ...
  const qEmi = useMemo(() => Math.round(emi(qAmount, qRate, qYears)), [qAmount, qRate, qYears]);

  // Comparison list of other partners
  const others: BankPreview[] = Object.entries(PARTNERS)
    .filter(([key]) => key !== k)
    .map(([key, p]) => ({ slug: key, name: p.name, range: p.range, minRate: p.minRate, maxRate: p.maxRate }));
      ...
      .then(async (res) => {
        if (!res.ok) {
          ...
          try {
            ...
            if (j?.errors) {
              const firstErr = Object.values(j.errors)[0] as string[] | undefined;
      ...
      <div className="bg-gray-50 min-h-screen">
        ...
        <main className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-10">
          <div className="rounded-xl border border-rose-200 bg-rose-50 p-5 text-rose-800">
            Unknown partner. <Link to="/" className="text-rose-700 underline">Go home</Link>
          </div>
        </main>
        <Footer />
      </div>
  ...
  const faqs: FaqItem[] = [
    {
      ...
      a: <>Most offers are floating (repo‑linked) and can change with RBI moves; fixed rate options may be available for specific durations.</>,
    },
    {
      q: 'Can I prepay without charges?',
      a: <>Many lenders allow part‑prepayment with minimal or no charges for floating‑rate loans. Always check your sanction letter.</>,
    },
    {
      q: 'What is a good LTV?',
      a: <>Up to 90% is typical. A lower LTV (higher down payment) can help you negotiate better terms and faster approvals.</>,
    },
  ];

  const globalMinRate = Math.min(...Object.values(PARTNERS).map(p => p.minRate));
  const globalMaxRate = Math.max(...Object.values(PARTNERS).map(p => p.maxRate));
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="mb-6">
          <div className="mb-6 flex items-center gap-2">
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-slate-900">{partner.name}  -  Home Loans</h1>
          </div>
          <p className="mt-2 text-slate-600">
            Indicative interest rate range: {partner.range}. Final rates depend on your profile and lender policy.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
          <div className="lg:col-span-2 space-y-6">
            <Card title="Overview of the bank">
              <div className="grid grid-cols-1 gap-6">
                <div>
                  <div className="text-sm font-semibold text-slate-900 mb-2">Quick EMI preview</div>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <label className="block">
                      ...
                      <input
                        ...
                        value={qAmount}
                        min={0}
                        onChange={(e) => setQAmount(Math.max(0, parseInt(e.target.value || '0', 10)))}
                      />
                    </label>
                    <label className="block">
                      ...
                      <input
                        ...
                        className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
                        value={qRate}
                        onChange={(e) => setQRate(Math.max(0, parseFloat(e.target.value || '0')))}
                      />
                      <div className="mt-1 text-[11px] text-slate-500">Typical: {partner.minRate}–{partner.maxRate}%</div>
                    </label>
                    <label className="block">
                      ...
                      <input
                        ...
                        className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
                        value={qYears}
                        onChange={(e) => setQYears(Math.max(1, parseInt(e.target.value || '1', 10)))}
                        min={1}
                        max={35}
                      />
                    </label>
                  ...
                  <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    <Kpi label="Approx. Interest" value={fmtINR(qEmi * 12 * qYears - qAmount)} />
                    <Kpi label="Processing fee (est.)" value={fmtINR(qAmount * 0.005)} hint="~0.5% of loan amount" />
                  </div>
                  <div className="mt-4">
                    <div className="text-xs font-medium text-slate-700 mb-1">Interest band vs market</div>
                    <RateRangeBar min={partner.minRate} max={partner.maxRate} globalMin={globalMinRate} globalMax={globalMaxRate} />
                  </div>
                </div>

                <div>
                  <div className="text-sm font-semibold text-slate-900 mb-2">Features & charges</div>
                  <ul className="space-y-3 text-sm text-slate-700">
                    <li className="rounded border border-slate-200 p-3">
                      <div className="font-medium text-slate-900">Interest rate</div>
                      <div>Typical band {partner.range}. Floating (repo-linked) in most cases; fixed options may exist.</div>
                      <div className="text-[11px] text-slate-500 mt-1">Benefit: Lower rates during easing cycles.</div>
                    </li>
                    <li className="rounded border border-slate-200 p-3">
                      <div className="font-medium text-slate-900">Processing fees</div>
                      <div>{partner.snapshot.processingFee}. Often negotiable during bank campaigns.</div>
                      <div className="text-[11px] text-slate-500 mt-1">Benefit: Watch for limited-time fee waivers.</div>
                    </li>
                    <li className="rounded border border-slate-200 p-3">
                      <div className="font-medium text-slate-900">Prepayment charges</div>
                      <div>{partner.snapshot.prepayment}. Floating-rate loans often allow part-prepay with minimal/no charges.</div>
                      <div className="text-[11px] text-slate-500 mt-1">Benefit: Reduce interest outgo by prepaying whenever possible.</div>
                    </li>
                    <li className="rounded border border-slate-200 p-3">
                      <div className="font-medium text-slate-900">Legal & technical</div>
                      <div>{partner.snapshot.legalTech || 'Legal & technical charges as per case'}</div>
                      <div className="text-[11px] text-slate-500 mt-1">Tip: Keep property docs ready for faster turnaround.</div>
                    </li>
                  </ul>
                </div>
              </div>
            ...
            <Card title="Key documents required">
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-slate-700">
                <li className="rounded border border-slate-200 p-3">KYC (PAN, Aadhaar)</li>
                <li className="rounded border border-slate-200 p-3">Income docs (salary slips/ITR) + bank statements</li>
                <li className="rounded border border-slate-200 p-3">Property docs (agreement/title/NOC as applicable)</li>
                <li className="rounded border border-slate-200 p-3">Employment proof & address proof</li>
              </ul>
            </Card>
          </div>
          ...
          <div>
            <Card title="Get a personalized home loan offer" subtitle={`From ${partner.name} & other partners`}>
              {!formDone ? (
                <form onSubmit={submitForm} className="space-y-3">
                  ...
                  <label className="block">
                    ...
                    >
                      {['Dehradun', 'Bangalore', 'Delhi NCR', 'Mumbai', 'Pune'].map((c) => (
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <label className="block">
                      ...
                      <input
                        ...
                        value={pLoanAmt}
                        min={0}
                        onChange={(e) => setPLoanAmt(Math.max(0, parseInt(e.target.value || '0', 10)))}
                        required
                      />
                    </label>
                    <label className="block">
                      ...
                      <input
                        ...
                        className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
                        value={pTenure}
                        onChange={(e) => setPTenure(Math.max(1, parseInt(e.target.value || '1', 10)))}
                        min={1}
                        max={35}
                        required
                      />
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <label className="block">
                      ...
                      <input
                        ...
                        value={pIncome}
                        min={0}
                        onChange={(e) => setPIncome(Math.max(0, parseInt(e.target.value || '0', 10)))}
                      />
                    </label>
                    <label className="block">
                      ...
                      <input
                        ...
                        value={pEmiObl}
                        min={0}
                        onChange={(e) => setPEmiObl(Math.max(0, parseInt(e.target.value || '0', 10)))}
                      />
                    </label>
                  </div>
                  ...
                  {formErr && (
                    <div className="rounded-md border border-rose-200 bg-rose-50 px-3 py-2 text-[13px] text-rose-700">{formErr}</div>
                  )}
                  ...
                  <div className="flex justify-end">
                    <button
                      type="submit"
                      disabled={submitting}
                      className={`rounded-md px-4 py-2 text-white ${submitting ? 'bg-emerald-400 cursor-not-allowed' : 'bg-emerald-600 hover:bg-emerald-700'}`}
                    >
            ...
            <div className="rounded-md border border-emerald-200 bg-emerald-50 p-4 text-emerald-800">
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <div className="text-xs mt-1">Our specialist will contact you shortly with curated options.</div>
                  <div className="mt-3 flex flex-wrap gap-2">
                    <Link to="/home-loans/emi-calculator" className="rounded-md bg-white px-3 py-1.5 text-xs text-emerald-700 border border-emerald-600 hover:bg-emerald-50">
                      Try EMI Calculator
                    </Link>
                    <Link to="/home-loans/eligibility-calculator" className="rounded-md bg-white px-3 py-1.5 text-xs text-emerald-700 border border-emerald-600 hover:bg-emerald-50">
                      Check Eligibility
                    </Link>
                  </div>
                </div>
  ...
  <CompareOffers partners={PARTNERS} selectedSlug={k} sampleAmount={qAmount} sampleYears={qYears} />

  {/* Guide + Carousel */ }
  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
    <Card title="Complete guide to home loans">
      <ol className="list-decimal pl-5 text-sm text-slate-700 space-y-1.5">
        <li>Assess budget: EMI comfort, down payment, and emergency fund.</li>
        <li>Check eligibility: income, FOIR, credit score, tenure.</li>
        <li>Compare offers: rate, fees, LTV, and prepayment policy.</li>
        <li>Collect documents: KYC, income, property, and employment proofs.</li>
        <li>Apply & verify: property valuation, legal checks, sanction.</li>
        <li>Disbursal: staged or full, depending on property status.</li>
        <li>Post‑disbursal: set up auto‑debit, track rate changes, plan prepayments.</li>
      </ol>
    </Card>

    {/* Explore other banks — CTA carousel (like Eligibility page) */}
    <Card title="Explore other banks" subtitle="Auto‑rotate. Hover to pause, or use arrows.">
  ...
  <p className="mt-3 text-xs text-slate-500">
    Disclaimer: Ranges are indicative. Offers subject to lender’s terms and credit evaluation.
  </p>
      </main >
  <Footer />
    </div >
