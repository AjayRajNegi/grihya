...
import { ChevronDown, HelpCircle, ChevronLeft, ChevronRight, CheckCircle2, Info } from 'lucide-react';
    ...
    <div>
      <div className="text-xs text-slate-700 mb-1 flex items-center gap-1.5">
        ...
        <Info className="h-3.5 w-3.5 text-slate-500">
          <title>LTV is the loan amount as a % of property value. Many lenders allow up to ~90%.</title>
        </Info>
      </div>
...
const FAQ: React.FC<{ items: FaqItem[]; idPrefix?: string }> = ({ items, idPrefix = 'apply' }) => {
...
type BankSlug = 'any' | 'hdfc' | 'bank-of-maharashtra' | 'nainital-bank' | 'icici';
...
const ApplyHomeLoan: React.FC = () => {
  ...
  // Derived
  const ltvPct = useMemo(() => (propertyValue > 0 ? (loanAmount / propertyValue) * 100 : 0), [loanAmount, propertyValue]);
  const estEmi = useMemo(() => Math.round(emi(loanAmount, rate, tenureYears)), [loanAmount, rate, tenureYears]);
  ...
  const bankHint = useMemo(() => {
    switch (preferredBank) {
      case 'hdfc': return 'HDFC typical range: 8.40% – 9.40%';
      case 'bank-of-maharashtra': return 'Bank of Maharashtra typical range: 8.35% – 9.35%';
      case 'nainital-bank': return 'Nainital Bank typical range: 8.60% – 9.60%';
      case 'icici': return 'ICICI typical range: 8.50% – 9.50%';
      default: return 'Select a preferred bank or keep Any';
    }
  }, [preferredBank]);
  ...
  const faqs: FaqItem[] = [
    {
      ...
      a: (
        <p>
          Our team will call you to verify details and match lender offers. You’ll get curated options from our partner banks, and we’ll guide you on documents and next steps.
        </p>
      ),
    },
    {
      q: 'Is my information safe?',
      a: <p>Yes. Your data is used only to assist with your loan application and is never sold to third parties.</p>,
    },
    {
      q: 'Can I change my preferred bank later?',
      a: <p>Absolutely. You can compare offers and switch your choice during the process.</p>,
    },
    {
      q: 'What is a good LTV?',
      a: <p>LTV up to 90% is typical. Lower LTVs (more down payment) may fetch better rates and faster approvals.</p>,
    },
  ];
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
        {!submitted ? (
          <>
            {/* Hero */}
            <div className="mb-6">
              <h1 className="text-2xl md:text-3xl font-bold text-slate-900">Apply Home Loan</h1>
              <p className="mt-2 text-slate-600">
                Share your details to get personalized offers from HDFC, Bank of Maharastra, Nainital Bank, ICICI and more. Fast, secure, and free.
              </p>
            </div>
              ...
              <div className="lg:col-span-2 space-y-6">
                <Card title="Contact details">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <label className="block">
                      <span className="text-xs font-medium text-slate-700">Phone (10 digits)</span>
                      <input
                        type="tel"
                        value={phone}
                        ...
                        required
                      />
                      <div className="mt-1 text-[11px] text-slate-500">We’ll call to confirm details and share offers.</div>
                    </label>
                    <label className="block">
                      ...
                      <input
                        ...
                        required
                      />
                      <div className="mt-1 text-[11px] text-slate-500">For sharing quotes and documentation checklist.</div>
                    </label>
                    <label className="block">
                ...
                <Card title="Loan details">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <label className="block">
                      <span className="text-xs font-medium text-slate-700">Property value (₹)</span>
                      <input
                        type="number"
                        value={propertyValue}
                        onChange={(e) => setPropertyValue(Math.max(0, parseInt(e.target.value || '0', 10)))}
                        className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
                        min={0}
                        required
                      />
                      <div className="mt-1 text-[11px] text-slate-500">Stamp duty/registration not included in loan by default.</div>
                    </label>
                    <label className="block">
                      ...
                      <input
                        type="number"
                        value={loanAmount}
                        onChange={(e) => setLoanAmount(Math.max(0, parseInt(e.target.value || '0', 10)))}
                        className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
                        min={0}
                        required
                      />
                      <div className="mt-1 text-[11px] text-slate-500">Try to keep LTV ≤ 90% for faster approvals.</div>
                    </label>
                    <label className="block">
                      ...
                      <input
                        type="number"
                        value={tenureYears}
                        onChange={(e) => setTenureYears(Math.max(1, parseInt(e.target.value || '1', 10)))}
                        className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm"
                        min={1}
                        max={35}
                        required
                      />
                      <div className="mt-1 text-[11px] text-slate-500">Longer tenure → lower EMI, but higher total interest.</div>
                    </label>
                    <label className="block">
                      <span className="text-xs font-medium text-slate-700">Expected rate (p.a.)</span>
                      <input
                        type="number"
                        step={0.1}
                        value={rate}
                        ...
                        required
                      />
                      <div className="mt-1 text-[11px] text-slate-500">Most offers are floating (repo‑linked); they can change with RBI moves.</div>
                    </label>
                    <label className="block">
                      <span className="text-xs font-medium text-slate-700">Employment type</span>
                      ...
                      <div className="mt-1 text-[11px] text-slate-500">Self‑employed may need additional financials (ITR, P&L).</div>
                    </label>
                    <label className="block">
                      <span className="text-xs font-medium text-slate-700">Monthly income (₹) (optional)</span>
                      <input
                        type="number"
                        value={monthlyIncome}
                        ...
                        min={0}
                      />
                      <div className="mt-1 text-[11px] text-slate-500">Helps us pre‑assess your eligibility and FOIR.</div>
                    </label>
                    <label className="block">
                      <span className="text-xs font-medium text-slate-700">Ongoing EMIs (₹/mo) (optional)</span>
                      <input
                        type="number"
                        value={existingEmi}
                        ...
                        min={0}
                      />
                      <div className="mt-1 text-[11px] text-slate-500">Includes credit cards converted to EMIs, personal loans, etc.</div>
                    </label>

                    <label className="block sm:col-span-2">
                      <span className="text-xs font-medium text-slate-700">Preferred bank</span>
                      <div className="mt-1 flex flex-wrap items-center gap-3 text-sm">
                        {(['any', 'hdfc', 'bank-of-maharashtra', 'nainital-bank', 'icici'] as BankSlug[]).map((b) => (
                          <label key={b} className="inline-flex items-center gap-2">
                            ...
                            <span className="capitalize">
                              {b === 'any' ? 'Any'
                                : b === 'bank-of-maharashtra' ? 'Bank of Maharashtra'
                                  : b === 'nainital-bank' ? 'Nainital Bank'
                                    : b.toUpperCase()}
                            </span>
                          </label>
                        ))}
                        <span className="text-[11px] text-slate-500">({bankHint})</span>
                      </div>
                    </label>
                  </div>

                  <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <Kpi label="Estimated EMI" value={fmtINR(estEmi)} hint={`@ ${rate}% for ${tenureYears} years`} />
                    <Kpi label="LTV" value={`${ltvPct.toFixed(1)}%`} hint={ltvPct > 90 ? 'May be restricted by lender' : 'Within common limits'} />
                    <Kpi label="Processing fee (est.)" value={fmtINR(pfEst)} hint="~0.5% of loan amount (varies by lender)" />
                  </div>
                ...
                <Card title="Your quick summary">
                  <div className="grid grid-cols-1 gap-3">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span>Expected EMI: <span className="font-semibold">{fmtINR(estEmi)}/mo</span></span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <CheckCircle2 className="h-4 w-4 text-emerald-600" />
                      <span>LTV: <span className="font-semibold">{ltvPct.toFixed(1)}%</span></span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <CheckCircle2 className="h-4 w-4 text-emerald-600" />
                      <span>Processing fee (est.): <span className="font-semibold">{fmtINR(pfEst)}</span></span>
                    </div>
                    <div className="text-[11px] text-slate-500">
                      Estimates are illustrative; final offer depends on profile and lender policy.
                    </div>
                  </div>
                </Card>
            ...
            <div className="mt-8">
              <Card title="Home Loan Eligibility  -  Quick checklist" subtitle="Typical benchmarks (vary by lender)">
            ...
            <div className="mt-8">
              <Card title="Understand the terms" subtitle="Short explanations of common terms">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-slate-700">
                  <div className="rounded-lg border border-slate-200 p-4">
                    <div className="font-medium text-slate-900">LTV (Loan‑to‑Value)</div>
                    <p className="mt-1">Loan amount as a % of property value. Higher LTV = smaller down payment; many lenders cap at ~90%.</p>
                  </div>
                  <div className="rounded-lg border border-slate-200 p-4">
                    <div className="font-medium text-slate-900">FOIR</div>
                    <p className="mt-1">Fixed Obligations to Income Ratio: portion of income available for EMIs after existing obligations.</p>
                  </div>
                  <div className="rounded-lg border border-slate-200 p-4">
                    <div className="font-medium text-slate-900">Floating vs Fixed Rate</div>
                    <p className="mt-1">Floating (repo‑linked) moves with RBI policy; fixed stays the same for a defined period.</p>
                  </div>
                  <div className="rounded-lg border border-slate-200 p-4">
                    <div className="font-medium text-slate-900">APR</div>
                    <p className="mt-1">Annual Percentage Rate reflects total cost including fees; helps compare offers beyond the headline rate.</p>
                  </div>
                  <div className="rounded-lg border border-slate-200 p-4">
                    <div className="font-medium text-slate-900">Pre‑EMI</div>
                    <p className="mt-1">Interest‑only payment before full disbursal (common in under‑construction properties).</p>
                  </div>
                  <div className="rounded-lg border border-slate-200 p-4">
                    <div className="font-medium text-slate-900">Foreclosure / Prepayment</div>
                    <p className="mt-1">Closing part/all of the loan early. Many lenders allow partial prepayment with minimal or no charges.</p>
                  </div>
                  <div className="rounded-lg border border-slate-200 p-4">
                    <div className="font-medium text-slate-900">Balance Transfer</div>
                    <p className="mt-1">Move loan to another bank at a lower rate; often combined with a top‑up loan.</p>
                  </div>
                  <div className="rounded-lg border border-slate-200 p-4">
                    <div className="font-medium text-slate-900">Sanction vs Disbursal</div>
                    <p className="mt-1">Sanction is approval (limit, rate, tenure). Disbursal is the actual payout to seller/builder.</p>
                  </div>
                  <div className="rounded-lg border border-slate-200 p-4">
                    <div className="font-medium text-slate-900">Co‑applicant</div>
                    <p className="mt-1">Adding a co‑applicant’s income can improve eligibility and rate offers.</p>
                  </div>
                  <div className="rounded-lg border border-slate-200 p-4">
                    <div className="font-medium text-slate-900">Insurance (Credit/Property)</div>
                    <p className="mt-1">Optional loan cover and property insurance may be bundled; not mandatory - compare costs.</p>
                  </div>
                  <div className="rounded-lg border border-slate-200 p-4">
                    <div className="font-medium text-slate-900">Processing Fee</div>
                    <p className="mt-1">One‑time charge (often 0.25%–1%) for processing the loan; negotiate where possible.</p>
                  </div>
                  <div className="rounded-lg border border-slate-200 p-4">
                    <div className="font-medium text-slate-900">Disbursement Linked Plan</div>
                    <p className="mt-1">For under‑construction homes, disbursals are linked to construction stages; EMIs rise as payouts increase.</p>
                  </div>
                </div>
              </Card>
            </div>
          ...
          <div className="max-w-2xl mx-auto">
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-6 text-emerald-900">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h2 className="text-xl font-semibold">Thanks, {fullName || 'there'}!</h2>
                  <p className="mt-1 text-sm">
                    We’ve received your request. Our loan expert will contact you shortly at {phone || 'your number'} and {email || 'your email'}.
                  </p>
                  <ul className="mt-3 list-disc pl-5 text-sm space-y-1">
                    <li>Keep your KYC and income documents handy.</li>
                    <li>We’ll share offers from HDFC, Bank of Maharashtra, Nainital Bank, ICICI and more.</li>
                    <li>No charges for applying via EasyLease.</li>
                  </ul>
                  <div className="mt-4 flex flex-wrap gap-2">
                    <Link to="/home-loans/emi-calculator" className="rounded-md bg-white px-3 py-1.5 text-sm text-emerald-700 hover:bg-emerald-100">
                      Try EMI Calculator
                    </Link>
                    <Link to="/home-loans/eligibility-calculator" className="rounded-md border border-emerald-700 px-3 py-1.5 text-sm text-emerald-800 hover:bg-emerald-100">
                      Check Eligibility
                    </Link>
                  </div>
                </div>
