...
import {
  MapPinIcon, BedIcon, BathIcon, SquareIcon, CheckIcon,
  WifiIcon, CoffeeIcon, CarIcon, TvIcon, HomeIcon,
  BuildingIcon, CalendarIcon, ChevronLeftIcon, ChevronRightIcon, Share2Icon, ChevronsUpDown as LiftIcon
} from 'lucide-react';
...
const absolutize = (u?: string | null) => {
  if (!u) return '';
  if (/^(?:[a-z][a-z0-9+.-]*:)?\/\//i.test(u) || u.startsWith('data:')) return u; // already absolute
...
const toProperty = (p: ApiProperty): ViewProperty => {
  ...
  return {
    ...
    preferred_tenants: (p.preferred_tenants ?? null) as ExtraFields['preferred_tenants'],
  };
};
...
const PropertyDetail: React.FC = () => {
  ...
  useEffect(() => {
    ...
    async function loadSimilar() {
      ...
      try {
        ...
        if (!res.ok) throw new Error(`Failed to load similar properties (${res.status})`);
        const json = await res.json();
        const items: ApiProperty[] = Array.isArray(json) ? json : json.data;
        const mapped = items.map(toProperty).filter(p => p.id !== property.id).slice(0, 3);
  ...
  async function handleShare() {
    ...
    const text = `Check this property on EasyLease: ${property.title} • ${property.location} • ₹${property.price.toLocaleString()}${property.for === 'rent' ? '/month' : ''}`;
    ...
    setShareNotice(ok ? 'Link copied to clipboard' : 'Unable to share. Copy the link manually.');
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 text-center">
        ...
        <p className="text-gray-600 mb-6">
          {err ? err : 'The property you are looking for does not exist or has been removed.'}
        </p>
        <Link
          to="/properties"
  ...
  const getAmenityIcon = (amenity: string) => {
    ...
    switch (a) {
      case 'wifi': return <WifiIcon className="h-5 w-5 text-[#2AB09C]" />;
      case 'parking': return <CarIcon className="h-5 w-5 text-[#2AB09C]" />;
      case 'tv': return <TvIcon className="h-5 w-5 text-[#2AB09C]" />;
      case 'ac': return <SnowflakeIcon className="h-5 w-5 text-[#2AB09C]" />;
      case 'gym': return <DumbbellIcon className="h-5 w-5 text-[#2AB09C]" />;
      case 'swimming pool': return <SwimmingIcon className="h-5 w-5 text-[#2AB09C]" />;
      case 'laundry': return <LaundryIcon className="h-5 w-5 text-[#2AB09C]" />;
      case 'security': return <ShieldIcon className="h-5 w-5 text-[#2AB09C]" />;
      case 'cafeteria': return <CoffeeIcon className="h-5 w-5 text-[#2AB09C]" />;
      case 'kitchen': return <KitchenIcon className="h-5 w-5 text-[#2AB09C]" />;
      case 'geyser':
      case 'gyser':
    ...
    <div className="bg-gray-50 min-h-screen py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex mb-6 text-sm text-gray-600">
          ...
          <Link to="/" className="hover:text-[#2AB09C]">Home</Link>
          <span className="mx-2">/</span>
          <Link to="/properties" className="hover:text-[#2AB09C]">Properties</Link>
          <span className="mx-2">/</span>
          <span className="text-gray-900 font-medium">{property.title}</span>
        </nav>

        {/* Gallery */}
        <PropertyGallery images={galleryImages} title={property.title} onImageClick={handleImageClick} />

        {/* Full-screen Image Modal */}
        {selectedImageIndex !== null && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/80" onClick={closeModal}>
            <div className="relative max-w-5xl w-full">
              <img
                ...
                onError={(e) => {
                  (e.target as HTMLImageElement).src = 'https://via.placeholder.com/1200x800?text=No+Image';
                }}
              />
              <button
                type="button"
          ...
          <div className="md:col-span-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                ...
                {property.furnishing && (
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <div className="font-medium capitalize">{property.furnishing}</div>
                    </div>
                  </div>
                )}
                ...
                {property.for === 'rent' && (
                  <div className="flex items-center">
                    ...
                    <div>
                      <div className="text-sm text-gray-600">Immediately Available</div>
                      <div className="font-medium">
                        {property.available_immediately == null
                          ? '—'
                          : property.available_immediately ? 'Yes' : 'No'}
                        {!property.available_immediately && property.available_from_date
                          ? ` ( From ${new Date(property.available_from_date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })} )`
                          : ''}
                      </div>
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="font-medium">
                        {property.ready_to_move == null
                          ? '—'
                          : property.ready_to_move ? 'Yes' : 'No'}
                        {!property.ready_to_move && property.possession_date
                          ? ` ( From ${new Date(property.possession_date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })} )`
                          : ''}
                      </div>
                    </div>
                  </div>
                ...
                {(property.for === 'rent' &&
                  (property.type === 'pg' || property.type === 'flat' || property.type === 'house')) && (
                    <div className="flex items-center">
                      <HomeIcon className="h-5 w-5 text-[#2AB09C] mr-3" />
                      <div>
                        <div className="text-sm text-gray-600">Preferred Tenants</div>
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <div className="font-medium capitalize">
                      {property.type === 'pg' ? 'PG Accommodation'
                        : property.type === 'flat' ? 'Apartment/Flat'
                          : property.type === 'house' ? 'Independent House/Villa'
            ...
            <div className="bg-white rounded-lg shadow-md p-6 mb-8">
              ...
              {(property.lat != null && property.lng != null) ? (
                ...
                <div className="text-gray-600 text-sm">
                  Map not available for this listing. Address: {property.display_label || property.formatted_address || property.location}
                </div>
              )}
            </div>
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              {loadingSimilar && <div className="text-gray-500 text-sm">Loading…</div>}
              {!loadingSimilar && similar.length === 0 && (
                <div className="text-gray-500 text-sm">No similar properties found.</div>
              )}
                ...
                {similar.map((sp) => (
                  ...
                  >
                    <div className="w-24 h-24 flex-shrink-0">
                      <img
                        src={sp.images?.[0] || 'https://via.placeholder.com/300x300?text=No+Image'}
                        alt={sp.title}
                    ...
                    <div className="p-3">
                      ...
                      <p className="text-[#2AB09C] font-medium text-sm">
                        ₹{sp.price.toLocaleString()}{sp.for === 'rent' ? '/month' : ''}
                      </p>
                    </div>
                  </Link>
                ))}
